...
# request_inf_zip=[]
# for i in range(1300):#300
#     for j in range(3,53,8):#3,84,10
#         request_inf = creat_request(G, j/200, 30, 0)#512bytes->32
#         request_inf_zip.append(request_inf)

c=nx.to_numpy_array(G,weight='capacity')
# np.save('request_inf_data_zip.npy', request_inf_zip)
np.save('netdata_test.npy', c)

# a = np.load('request_inf_data_zip.npy', allow_pickle=True)
b=np.load('netdata_test.npy',allow_pickle=True)
...
print(G)
for (i,j) in d.edges():
    k=d[i][j]['weight']-G[i+1][j+1]['capacity']
    print(k)