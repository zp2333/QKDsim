...
for topo_c in topo_choice:
    topology_data=creat_net("archive/"+topo_c+'.gml')
    supplement_key={
    ...
    a = np.load(store_path+topo_c+'/'+data_choice+'_request_inf_data_zip.npy', allow_pickle=True)
    j=0
    for request_inf in a:
        ...
        result['request2'].append(len(request_inf1['request_data']))
        print("****************************************************************")
        compare_route1(G3, length_path,request_inf,request_inf3,supplement_key0,result)
        ...
        np.save(store_path+topo_c+'/'+data_choice+'/G1/'+str(j)+'_netdata.npy', a1)
        np.save(store_path+topo_c+'/'+data_choice+'/R1/'+str(j)+'_requestdata.npy', request_inf1)
        a2=nx.to_numpy_array(G2,weight='capacity')
        np.save(store_path+topo_c+'/'+data_choice+'/G2/'+str(j)+'_netdata.npy', a2)
        np.save(store_path+topo_c+'/'+data_choice+'/R2/'+str(j)+'_requestdata.npy', request_inf2)
        a3=nx.to_numpy_array(G3,weight='capacity')
        np.save(store_path+topo_c+'/'+data_choice+'/G3/'+str(j)+'_netdata.npy', a3)
        np.save(store_path+topo_c+'/'+data_choice+'/R3/'+str(j)+'_requestdata.npy', request_inf3)
        np.save(store_path+topo_c+'/'+data_choice+'/result/'+str(j)+'_netlog.npy', result)
