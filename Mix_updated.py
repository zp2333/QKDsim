...
def improve_route(G, length_path, request_inf, supplement_key,result):
    ...
    if len(request_inf['request_data']) != 0:
        ...
        for (i,j) in G.edges():
            ...
            if x!=0:
                alpha.append(x)
                link_use_num+=1
            elif G_temp[j][i]['capacity']-G[j][i]['capacity']!=0:
                link_use_num+=1
    ...
    result['var1'].append(np.std(test,ddof=1))
...
def compare_route2(G, length_path, request_inf, supplement_key,result):
    ...
    result['var3'].append(np.std(test,ddof=1))
    update_key(G, supplement_key)
...
topology_data=creat_net("archive\Chinanet.gml")
...
for i in range(100):
    for j in range(3,120,10):#3,120,10
        result={
            "request1":[],
            "abandon1":[],
            "min1":[],
        ...
        G2 = copy.deepcopy(G)
        G3 = copy.deepcopy(G)
        #request_inf = {'request_data': [512, 512, 512, 512, 512, 512], 'request_source': [7, 24, 24, 25, 25, 38], 'request_destination': [36, 36, 2, 1, 19, 5]}
        request_inf = creat_request(G, j/100, 32, 0)#512bytes
        # request_inf = {
        #     'request_data':[620,308,154,154],
        #     'request_source':[15,1,1,1],
        #     'request_destination':[16,8,2,3],
        # }
        request_inf1 = copy.deepcopy(request_inf)
        request_inf2 = copy.deepcopy(request_inf)
        request_inf3 = copy.deepcopy(request_inf)
        print("This is ",i,".",j," test")
        ...
        print("----------------------------------------------------------------")
        compare_route2(G3, length_path, request_inf3, supplement_key0,result)
        result['request3'].append(len(request_inf['request_data'])-result['abandon3'][-1])
        print("****************************************************************")
        compare_route1(G2, length_path, request_inf2,request_inf2, supplement_key0,result)
