...
for topo_c in topo_choice1:
    ...
    print("****************************************************************")
    for brust_num in range(5,40,5):
        writer = pd.ExcelWriter(store_path+topo_c+'/'+data_choice+'_'+'result.xlsx')
        for i in range(500):#500
            ...
            request_inf=creat_request3(G,brust_num,32,0)
            request_inf1 = copy.deepcopy(request_inf)
            request_inf2 = copy.deepcopy(request_inf)
            request_inf3 = copy.deepcopy(request_inf)
            ...
            print("request is",request_inf)
            print("request_number is",len(request_inf['request_data']))
            result['error_link'].append(brust_num)
            improve_route_iteration(G11, length_path0, request_inf1, supplement_key0,result)
            result['request1'].append(len(request_inf['request_data']))
            print("----------------------------------------------------------------")
            compare_route2(G22, length_path0, request_inf2, supplement_key0,result)
            result['request2'].append(len(request_inf['request_data']))
            print("****************************************************************")
            compare_route1(G33, length_path0,request_inf,request_inf3,supplement_key0,result)
            ...
            np.save(store_path+topo_c+'/'+data_choice+'/G1/'+str(i)+str(brust_num)+'_netdata.npy', a1)
            np.save(store_path+topo_c+'/'+data_choice+'/R1/'+str(i)+str(brust_num)+'_requestdata.npy', request_inf1)
            a2=nx.to_numpy_array(G22,weight='capacity')
            np.save(store_path+topo_c+'/'+data_choice+'/G2/'+str(i)+str(brust_num)+'_netdata.npy', a2)
            np.save(store_path+topo_c+'/'+data_choice+'/R2/'+str(i)+str(brust_num)+'_requestdata.npy', request_inf2)
            a3=nx.to_numpy_array(G33,weight='capacity')
            np.save(store_path+topo_c+'/'+data_choice+'/G3/'+str(i)+str(brust_num)+'_netdata.npy', a3)
            np.save(store_path+topo_c+'/'+data_choice+'/R3/'+str(i)+str(brust_num)+'_requestdata.npy', request_inf3)
            np.save(store_path+topo_c+'/'+data_choice+'/result/'+str(i)+str(brust_num)+'_netlog.npy', result)
