...
length_path = 20
store_path='rowdata/3.2jhgx_num_5/'
...
a = np.load('request_inf_data_zip_5.npy', allow_pickle=True)
# a=[]
...
# 8, 40, 8, 8, 53, 1, 14, 40, 8, 1, 1, 24, 1, 53, 17, 8, 14, 33, 24, 40, 33, 62, 33, 14, 1, 8, 14, 24, 17, 24, 24, 24, 14, 24, 33, 62, 40, 17, 17, 40, 8, 24, 8, 17, 33, 53]}
    ...
    result['request1'].append(len(request_inf['request_data'])-result['abandon1'][-1]-1)
    print("----------------------------------------------------------------")
    ...
    result['request2'].append(len(request_inf1['request_data'])-1)
    print("****************************************************************")
    improve_route_without_max(G3, length_path, request_inf3,supplement_key0,result)
    result['request3'].append(len(request_inf1['request_data'])-1)
    print("################################################################")
    ...
    np.save(store_path+'G2/'+str(j)+'_netdata.npy', a2)
    np.save(store_path+'R2/'+str(j)+'_requestdata.npy', request_inf2)
    a3=nx.to_numpy_array(G3,weight='capacity')
    np.save(store_path+'G3/'+str(j)+'_netdata.npy', a3)
    np.save(store_path+'R3/'+str(j)+'_requestdata.npy', request_inf3)
