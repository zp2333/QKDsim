...
length_path = 20
store_path='rowdata/3.2jhgx_vol/'
...
# a=[]
# b={'request_data': [30, 155, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30], 'request_source': [1, 4, 8, 8, 15, 16, 17, 
# 17, 17, 17, 17, 17, 17, 17, 24, 24, 24, 24, 24, 24, 33, 33, 33, 33, 40, 40, 40, 40, 40, 40, 40, 53, 53, 62, 62, 62], 'request_destination': [10, 1, 35, 19, 42, 17, 61, 2, 36, 63, 29, 2, 21, 33, 31, 10, 9, 56, 16, 33, 29, 62, 8, 11, 67, 14, 43, 13, 56, 70, 55, 29, 48, 65, 71, 7]}
# a.append(b)
r_num=11
for vol in range(80,129,16):
    j=0
    a_temp=[]
    for request_inf in a:
        if len(request_inf['request_data'])==r_num:
                new_list=[vol/4 if i ==32 else i for i in request_inf['request_data']]
                new_list_temp=[]
                new_source_temp=[]
                ...
                for i in range(len(new_list)):
                    if new_list[i]<=100:
                        ...
                        new_source_temp.append(request_inf['request_source'][i])
                        new_destination_temp.append(request_inf['request_destination'][i])
                new_request={
                    'request_data': new_list+new_list_temp+new_list_temp+new_list_temp, 'request_source': request_inf['request_source']+new_source_temp+new_source_temp+new_source_temp, 'request_destination': request_inf['request_destination']+new_destination_temp+new_destination_temp+new_destination_temp
                }
                a_temp.append(new_request)
    for request_inf in a_temp:
        result={
            "request1":[],
            "abandon1":[],
            "min1":[],
    ...
    # 8, 40, 8, 8, 53, 1, 14, 40, 8, 1, 1, 24, 1, 53, 17, 8, 14, 33, 24, 40, 33, 62, 33, 14, 1, 8, 14, 24, 17, 24, 24, 24, 14, 24, 33, 62, 40, 17, 17, 40, 8, 24, 8, 17, 33, 53]}
        ...
        request_inf1 = copy.deepcopy(request_inf)
        print("vol is",vol)
        print("This is ",j,'/',len(a_temp)," test")
        j=j+1
        ...
        result['request1'].append(vol)
        print("----------------------------------------------------------------")
        request_inf2 = copy.deepcopy(request_inf1)
        request_inf3 = copy.deepcopy(request_inf1)
        compare_route2(G2, length_path, request_inf2, supplement_key0,result)
        result['request2'].append(vol)
        print("****************************************************************")
        improve_route_without_max(G3, length_path, request_inf3,supplement_key0,result)
        result['request3'].append(vol)
        print("################################################################")
        ...
        np.save(store_path+'G1/'+str(vol)+'_'+str(j)+'_netdata.npy', a1)
        np.save(store_path+'R1/'+str(vol)+'_'+str(j)+'_requestdata.npy', request_inf1)
        a2=nx.to_numpy_array(G2,weight='capacity')
        np.save(store_path+'G2/'+str(vol)+'_'+str(j)+'_netdata.npy', a2)
        np.save(store_path+'R2/'+str(vol)+'_'+str(j)+'_requestdata.npy', request_inf2)
        a3=nx.to_numpy_array(G3,weight='capacity')
        np.save(store_path+'G3/'+str(vol)+'_'+str(j)+'_netdata.npy', a3)
        np.save(store_path+'R3/'+str(vol)+'_'+str(j)+'_requestdata.npy', request_inf3)
        np.save(store_path+'result/'+str(vol)+'_'+str(j)+'_netlog.npy', result)
