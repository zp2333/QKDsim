...
r_num=10
for vol in range(0,121,5):
    j=0
    a_temp=[]
    for request_inf in a:
        if len(request_inf['request_data'])==r_num:
                new_list=[vol if i ==30 else i for i in request_inf['request_data']]
                new_request={
                    'request_data': new_list, 'request_source': request_inf['request_source'], 'request_destination': request_inf['request_destination']
                }
